...
# for some indicators it makes sense to discard the last value:
if(data_source %in% c("survstat")){
  discard_last_value <- TRUE
}else{
  discard_last_value <- FALSE
}
  ...
  for (disease in diseases) {
    ...
    for(loc in locations){
      ...
      for (j in seq_along(quantile_levels_nowcast)) {
        nc_temp <- subset(nc, quantile == quantile_levels_nowcast[i])
        ...
        # discard last value if desired:
        if(discard_last_value){
          ts_temp <- ts_temp[-nrow(ts_temp), ]
        }
        # generate forecasts:
        sts_temp <- sts(c(ts_temp$value, rep(NA, max_horizon)), start = c(ts_temp$year[1], ts_temp$week[1]))
      ...
      # get quantiles, format and store:
