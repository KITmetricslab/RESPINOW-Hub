...
# define data source: only AGI has regions
data_sources <- c("agi")
...
for(data_source in data_sources){
  ...
  for(i in seq_along(forecast_dates)){
    ...
    for (disease in diseases) {
      ...
      # identify locations:
      locs <- unique(triangles[[disease]]$location)
      locs <- locs[locs != "DE"]
      ...
      ###################################################
      # generate nowcasts for locations:
      ...
      mu_array <- var_array <- array(dim = c(length(locs), 
                                             max_horizon, 
      ...
      for (j in seq_along(quantile_levels_nowcast)) {
        # for all locations:
        for(loc in locs){
          nc_temp <- subset(all_nc, quantile == quantile_levels_nowcast[j] & location == loc)
          dates_to_replace <- sort(unique(nc_temp$target_end_date))
          values_to_replace <- nc_temp$value[order(nc_temp$target_end_date)]
          
          ts_temp$value[ts_temp$date %in% dates_to_replace & ts_temp$location == loc] <- values_to_replace
        }
        
        # set up a matrix for obsrvations incl. nowcast, to be passed to sts
        vect_DE_BW <- subset(ts, location == "DE-BW")$value
        n_observed <- length(vect_DE_BW)
        matr_observed <- matrix(NA, ncol = length(locs), nrow = n_observed)
        colnames(matr_observed) <- locs
        for(i in seq_along(locs)){
          loc <- locs[i]
          matr_observed[, i] <- subset(ts_temp, location == loc)$value
        }
        ...
        ctrl <- list(end = # list(f = addSeason2formula(~0 + fe(1, unitSpecific = TRUE),
                     ...
                     ar = list(f = addSeason2formula(~0 + fe(1, unitSpecific = TRUE),
                                                     period = 52.25, S = rep(1, length(locs)))),
                     family = "NegBinM",
                     par_lag = 0.5,
      ...
      for(loc in locs){
        ...
        mu_matrix <- mu_array[loc, , ]
        var_matrix <- var_array[loc, , ]
      ...
      # get forecasts for national level and ge groups (done separately)
      fc_pooled <- read.csv(fl, colClasses = c("forecast_date" = "Date",
                                               "target_end_date" = "Date"))
      fc_pooled <- subset(fc_pooled, location == "DE")
      # append:
